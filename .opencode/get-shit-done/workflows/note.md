...
- **Global scope**: `F:/xxx/xxx/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/xxx/xxx/.opencode/notes/`)
...
2. Glob `F:/xxx/xxx/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (F:/xxx/xxx/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/xxx/xxx/.opencode/notes/` — works in any directory
